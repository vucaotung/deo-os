...
Write-Host "`n=== [1/4] Show 8 context files currently loaded for deo ===" -ForegroundColor Cyan
docker exec goclaw-postgres-1 psql -U goclaw -d goclaw -c @"
SELECT file_name, length(content) AS bytes, acf.updated_at
...
WHERE session_id LIKE 'agent:deo:condeobot:%' OR session_id LIKE 'deo:%';
DELETE FROM episodic_summaries
WHERE session_id LIKE 'agent:deo:condeobot:%' OR session_id LIKE 'deo:%';
SELECT COUNT(*) AS remaining_memory FROM memory_chunks WHERE session_id LIKE '%deo%';
"@
...
Write-Host "`n=== [4/4] Re-apply tools + credentials (post-restart) ===" -ForegroundColor Cyan
docker exec -u root goclaw-goclaw-1 sh /app/workspace/setup_credentials.sh

Write-Host "`n✅ Done. Now test on Telegram with a DIFFERENT topic to bypass session match:" -ForegroundColor Green
Write-Host ""
